...
UniCore **canonicalizes** the schema on write. The canonicalized form is what gets stored and returned. Canonicalization:

- injects `"additionalProperties": false` recursively into every object node
- injects OB-mandated `"required"` arrays at the appropriate nesting levels (for OB templates; see below)

This means callers do not need to include `additionalProperties` or manage `required` arrays for OB fields themselves — the system handles it. The returned schema will contain these injected fields.
