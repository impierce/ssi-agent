...
mod tests {
    ...
    use agent_shared::config::{config, set_config, SupportedDidMethod, ToggleOptions};
    use agent_store::{identity_state, in_memory::InMemory};

    #[serial_test::serial]
    #[tokio::test]
    async fn linked_vp_dispatches_service_and_document_commands() {
        let original = config().clone();
        set_config().public_url = "https://identity.example.org/".parse().unwrap();
