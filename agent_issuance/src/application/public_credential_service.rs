...
impl DataAccessService {
    pub async fn get_public_credential(
        ...
        let credential = query_handler(credential_id, &state.query.credential)
            .await
            .map_err(|e| DataAccessServiceError::QueryError(format!("Failed to get credential from storage: {e}")))?
        ...
        let credential_claims = get_unverified_jwt_claims(&signed_credential).ok_or(
            DataAccessServiceError::InvalidJwtError("Failed to get credential JWT claims".to_string()),
        )?;
        ...
        let credential_subject_id = credential_claims
            ...
            .ok_or(DataAccessServiceError::InvalidJwtError(
        ...
        let jwt_header = decode_header(&data_access_consent_token)
            .map_err(|e| DataAccessServiceError::InvalidJwtError(format!("Failed to decode JWT header: {e}")))?;

        let kid = jwt_header
            .kid
            .ok_or_else(|| DataAccessServiceError::InvalidJwtError("JWT header does not contain a kid".to_string()))?;
        ...
        let public_key = state.subject.resolve_public_key(&kid).await.map_err(|_| {
            DataAccessServiceError::DACTError("Failed to fetch public key for DACT JWT verification".to_string())
        })?;
        ...
        decode::<serde_json::Value>(&data_access_consent_token, &decoding_key, &validation)
            .map_err(|e| DataAccessServiceError::InvalidJwtError(format!("JWT verification failed: {}", e)))?;
