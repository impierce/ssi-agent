...
use agent_authorization::{
    domain::authorization_server_config::aggregate::AuthorizationServerConfig,
    state::{AuthorizationState, AUTHORIZATION_SERVER_CONFIG_ID},
};
use axum::{
    extract::{Json, State},
    http::StatusCode,
...
pub(crate) async fn oauth_authorization_server(State(state): State<AuthorizationState>) -> Result<Response, ApiError> {
    match query_handler(AUTHORIZATION_SERVER_CONFIG_ID, &state.query.authorization_server_config)
        .await
        .expect("FIXME")
    {
        Some(AuthorizationServerConfig {
            // FIXME!!
            authorization_server_config_id: authorization_server_metadata,
            ..
