...
use agent_issuance::{
    ...
    state::{self, ApplicationState, CQRS},
};
use axum::{
    extract::FromRef,
    routing::{get, post},
...
#[derive(Clone)]
pub(crate) struct AggregateHandler<A, V>(pub(crate) state::AggregateHandler<A, V>)
where
    A: Aggregate,
    V: View<A>;

impl<A, V> std::ops::Deref for AggregateHandler<A, V>
where
    A: Aggregate,
    V: View<A>,
{
    type Target = state::AggregateHandler<A, V>;
...
impl FromRef<ApplicationState> for AggregateHandler<Offer, OfferView> {
    fn from_ref(application_state: &ApplicationState) -> AggregateHandler<Offer, OfferView> {
        AggregateHandler(application_state.offer.clone())
    }
}

impl FromRef<ApplicationState> for AggregateHandler<Credential, CredentialView> {
    fn from_ref(application_state: &ApplicationState) -> AggregateHandler<Credential, CredentialView> {
        AggregateHandler(application_state.credential.clone())
    }
}

impl FromRef<ApplicationState> for AggregateHandler<ServerConfig, ServerConfigView> {
    fn from_ref(application_state: &ApplicationState) -> AggregateHandler<ServerConfig, ServerConfigView> {
...
mod tests {
    ...
    pub async fn create_unsigned_credential(state: ApplicationState) -> String {
        state
            .credential
