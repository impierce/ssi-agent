...
use identity_iota::{
    credential::KeyBindingJWTValidationOptions, did::DID as _, document::DIDUrlQuery, verification::jwk::Jwk,
};
            ...
            VerifyAuthorizationResponse {
                ...
                match authorization_response {
                    ...
                    GenericAuthorizationResponse::OID4VP(oid4vp_authorization_response) => {
                        let sd_jwt_vc: SdJwtVc = match &oid4vp_authorization_response.extension.oid4vp_parameters {
                            Oid4vpParams::Params {
                                vp_token: OneOrManyVpToken::One(vp_token),
                                ..
                            } => vp_token.parse().unwrap(),
                        ...
                        let did_url = identity_iota::did::DIDUrl::parse(kid).unwrap();
                        ...
                        let jwk = verification_method.data().public_key_jwk().unwrap();

                        info!("jwk HERE: {}", serde_json::to_string_pretty(jwk).unwrap());
                        ...
                        let vp_token = sd_jwt_vc.to_string();
